// build-page.mjs — put the GATED ledger guard inside the page.
//
// fallslot is a single offline file, so the kernel is inlined rather than imported — and inlined
// VERBATIM, or the code that was gated and the code that refuses a double ledger would be two
// different programs.
import { readFileSync, writeFileSync } from 'node:fs';

const OPEN = '/* __LEDGER_KERNEL__ */';
const CLOSE = '/* __END_LEDGER_KERNEL__ */';

// Only the export KEYWORD goes. The \r? matters: these files are CRLF and `.` stops at \r.
const kernel = readFileSync('ledger.mjs', 'utf8')
  .replace(/^#!.*\r?\n/, '')
  .replace(/^import[^\n]*\n/gm, '')
...
for (const fn of ['function pence', 'function validateJournal', 'function trialBalance']) {
...
console.log(`index.html — ledger guard inlined, ${kernel.split('\n').length} lines, page ${(out.length / 1024).toFixed(0)}KB`);
